...
def test_get_system_info_linux_bash(
...
@patch(
    "os.environ.get",
    side_effect=lambda k, default=None: {"SHELL": "/usr/local/bin/zsh"}.get(k, default),
)
def test_get_system_info_macos_zsh(
    mock_environ_get, mock_python_version, mock_getcwd, mock_machine, mock_system
):
    system_info = get_system_info()
    assert isinstance(system_info, SystemInfo)
...
def test_get_system_info_unknown_shell(
    mock_environ_get, mock_python_version, mock_getcwd, mock_machine, mock_system
):
    system_info = get_system_info()
    assert isinstance(system_info, SystemInfo)
...
def test_get_system_info_windows_git_bash(
    mock_python_version, mock_environ_get, mock_getcwd, mock_machine, mock_system
):
    system_info = get_system_info()
    assert isinstance(system_info, SystemInfo)
    assert system_info.os_name == "Windows"
