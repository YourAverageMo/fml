...
def test_initialize_ai_service_valid_gemini_model(
        mock_gemini_service_class, mock_os_getenv,
    ...
    mock_gemini_service_class.assert_called_once_with(
        ...
        model=model_name)
    assert service is mock_gemini_service_class.return_value  # Ensure the returned service is the mocked instance


def test_initialize_ai_service_unsupported_model(mock_gemini_service_class,
                                                 mock_os_getenv,
                                                 mock_importlib_import_module):
    """
    ...
    assert f"Supported models are: {', '.join(MODELS.keys())}" in str(
        exc_info.value)  # Ensure supported models are listed dynamically
...
def test_initialize_ai_service_missing_gemini_api_key(
    ...
    assert "API key environment variable 'GEMINI_API_KEY' not set for model 'gemini-1.5-flash'." in str(
        exc_info.value)
    mock_gemini_service_class.assert_not_called(
    )  # Ensure service is not instantiated
