...
class SystemInfo(BaseModel):
    os_name: str = Field(..., description="The name of the operating system (e.g., 'Linux', 'Windows', 'Darwin').")
    shell: str = Field(..., description="The default shell being used (e.g., 'bash', 'zsh', 'cmd.exe').")
    cwd: str = Field(..., description="The current working directory.")
    architecture: str = Field(..., description="The system architecture (e.g., 'x86_64', 'arm64').")
    python_version: Optional[str] = Field(None, description="The Python version being used.")

class AIContext(BaseModel):
    system_info: Optional[SystemInfo] = Field(None, description="Information about the user's system environment.")
